...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_domain_dictionaries ON domain_dictionaries(scope_type, scope_key)")

    cols = {r[1] for r in conn.execute("PRAGMA table_info(approved_relations)").fetchall()}
    if cols and 'from_col_norm' not in cols:
        conn.execute("ALTER TABLE approved_relations ADD COLUMN from_col_norm TEXT NOT NULL DEFAULT ''")
        conn.execute("UPDATE approved_relations SET from_col_norm=COALESCE(from_col,'') WHERE from_col_norm='' OR from_col_norm IS NULL")
    cols = {r[1] for r in conn.execute("PRAGMA table_info(approved_relations)").fetchall()}
    if cols and 'to_col_norm' not in cols:
        conn.execute("ALTER TABLE approved_relations ADD COLUMN to_col_norm TEXT NOT NULL DEFAULT ''")
        conn.execute("UPDATE approved_relations SET to_col_norm=COALESCE(to_col,'') WHERE to_col_norm='' OR to_col_norm IS NULL")
    conn.commit()
...
def approve_relation(profile_id: str, relation: dict, notes: str = '') -> dict:
    ...
    with _conn() as conn:
        existing = conn.execute(
            ...
            (profile_id, payload['from_table'], payload['from_col_norm'], payload['to_table'], payload['to_col_norm'], payload['rel_type'])
        ).fetchone()
        if existing:
            conn.execute(
                ...
                (payload['from_col'], payload['to_col'], payload['score'], payload['evidence'], payload['signal'], payload['notes'], payload['raw_payload'], payload['created_at'], existing['id'])
            )
...
def get_dictionary(scope_type: str, scope_key: str) -> Optional[dict]:
    with _conn() as conn:
        row = conn.execute('SELECT * FROM domain_dictionaries WHERE scope_type=? AND scope_key=?', (scope_type, scope_key)).fetchone()
    return _row_to_dict(row) if row else None


def save_dictionary(scope_type: str, scope_key: str, dictionary: dict, industry: str = 'other', subdomain: str = '', region: str = '', notes: str = '') -> dict:
    ...
    with _conn() as conn:
        existing = conn.execute('SELECT id, created_at FROM domain_dictionaries WHERE scope_type=? AND scope_key=?', (scope_type, scope_key)).fetchone()
        if existing:
            payload['id'] = existing['id']
            payload['created_at'] = existing['created_at']
            conn.execute(
                'UPDATE domain_dictionaries SET industry=?, subdomain=?, region=?, dictionary_json=?, notes=?, updated_at=? WHERE id=?',
                (payload['industry'], payload['subdomain'], payload['region'], payload['dictionary_json'], payload['notes'], payload['updated_at'], payload['id'])
            )
        else:
            conn.execute(
...
def delete_dictionary(scope_type: str, scope_key: str) -> bool:
    with _conn() as conn:
        cur = conn.execute('DELETE FROM domain_dictionaries WHERE scope_type=? AND scope_key=?', (scope_type, scope_key))
        conn.commit()
        return cur.rowcount > 0


def resolve_dictionary(industry: str = 'other', subdomain: str = '', region: str = '') -> dict:
    ...
    for scope_type, scope_key in candidates:
        ...
        if item:
            ...
            matched.append({'scope_type': scope_type, 'scope_key': scope_key, 'term_count': len(item.get('dictionary_json') or {}), 'notes': item.get('notes','')})
    return {'dictionary': merged, 'matched_scopes': matched, 'resolution_order': candidates}
