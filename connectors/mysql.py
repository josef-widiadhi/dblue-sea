...
class MySQLConnector(BaseConnector):
    name="mysql"; display_name="MySQL / MariaDB"; icon="🐬"; default_port=3306; category="relational"
    ...
    def _conn(self,host,port,dbname,user,password,**kw):
        import pymysql
        return pymysql.connect(host=host,port=int(port),db=dbname,user=user,password=password,
    ...
    def test_connection(self,host,port,dbname,user,password,**kw):
        try:
            ...
            n=cur.fetchone()["n"]; conn.close(); return {"ok":True,"table_count":n}
    ...
    def get_table_list(self,host,port,dbname,user,password,**kw):
        ...
        r=[x["TABLE_NAME"] for x in cur.fetchall()]; conn.close(); return r

    def get_table_detail(self,table,host,port,dbname,user,password,**kw):
        conn=self._conn(host,port,dbname,user,password); cur=conn.cursor()
        cur.execute("SELECT COLUMN_NAME,DATA_TYPE,IS_NULLABLE,COLUMN_DEFAULT,COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=%s AND TABLE_NAME=%s ORDER BY ORDINAL_POSITION",(dbname,table))
    ...
    def extract_schema(self,host,port,dbname,user,password,schema=None,**kw):
        conn=self._conn(host,port,dbname,user,password); cur=conn.cursor()
        cur.execute("""SELECT TABLE_NAME,COLUMN_NAME,DATA_TYPE,COLUMN_TYPE,IS_NULLABLE,COLUMN_DEFAULT,
                       CASE WHEN COLUMN_KEY='PRI' THEN 1 ELSE 0 END as is_pk
                       FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=%s ORDER BY TABLE_NAME,ORDINAL_POSITION""",(dbname,))
        raw=cur.fetchall()
        cur.execute("""SELECT TABLE_NAME,COLUMN_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME
                       FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA=%s AND REFERENCED_TABLE_NAME IS NOT NULL""",(dbname,))
